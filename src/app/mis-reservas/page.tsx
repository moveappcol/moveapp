...
export default async function MisReservasPage() {
  ...
  const t = getDictionary(locale).misReservas;
  ...
  return (
    <section className="mx-auto max-w-3xl px-4 py-16 sm:px-6">
      ...
      {enriched.length === 0 ? (
        <p className="mt-6 font-body text-sm text-move-green/60">{t.sinReservas}</p>
      ) : (
              ...
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <p className="font-heading text-base font-semibold text-move-green">
                    {r.clase?.name ?? t.clase} — {r.gym?.name ?? t.gimnasio}
                  </p>
                  <p className="mt-1 font-body text-sm text-move-green/60">
                    {r.fecha
                      ? new Date(r.fecha).toLocaleString(locale === "en" ? "en-US" : "es-CO", {
